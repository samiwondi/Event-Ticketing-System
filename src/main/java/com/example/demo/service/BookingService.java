...
public class BookingService {
    ...
    public Reservation holdSeats(UUID eventId, String customerEmail, List<UUID> seatIds) {
        ReentrantLock lock = eventLocks.computeIfAbsent(eventId, k -> new ReentrantLock(true));
        lock.lock();
        try {
            ...
            var seats = seatIds.stream()
                    .map(id -> allSeats.stream().filter(s -> s.getId().equals(id)).findFirst()
                            .orElseThrow(() -> new InvalidSeatException("Seat " + id + " not found in this venue")))
            ...
            var reservationSeats = seats.stream()
                    .map(seat -> new ReservationSeat(reservationId, seat.getId(), Money.zero("USD"), DiscountType.NONE))
                    .collect(Collectors.toList());

            var reservation = new Reservation(
                    reservationId, eventId, customerEmail,
                    ReservationStatus.HOLD, now, null, holdExpires, reservationSeats
            );
