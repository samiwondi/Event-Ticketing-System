...
public class ReportService {
        ...
        public ReportService(ReservationRepository reservationRepository,SeatRepository seatRepository,EventRepository eventRepository) {
        ...
        public Map<UUID, Long> seatsPerEvent() {
                ...
                return events.stream().collect(Collectors.toMap(
                        event -> event.getId(),
                        event -> (long) seatRepository.findByVenueId(event.getVenueId()).size()
                ));
        }

        public Map<UUID, Map<String, Long>> reservationsPerEvent() {
                var allReservations = reservationRepository.findAll();
                return allReservations.stream().collect(Collectors.groupingBy(
        ...
        return "Seat report for event: " + event.getTitle() + "\n" +
                "Total seats: " + allSeats.size() + "\n" +
                "Reserved seats: " + reservedSeatIds.size() + "\n" +
                "Available seats: " + (allSeats.size() - reservedSeatIds.size());
        }
}